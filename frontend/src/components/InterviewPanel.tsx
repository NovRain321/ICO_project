...
export default function InterviewPanel({
  ...
  if (interviews.length === 0) {
    return (
      <div className="bg-white rounded-2xl p-12 text-center">
        ...
        <h3 className="text-xl font-semibold text-slate-700 mb-2">暂无复盘记录</h3>
        <p className="text-slate-500 mb-6">开始案件复盘，获取专业研判评估</p>
        <motion.button
          onClick={onStartInterview}
          className="px-6 py-3 bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-xl font-medium shadow-lg shadow-amber-500/30"
  ...
  return (
    <div className="space-y-6">
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-slate-800">复盘表现趋势</span>
            </div>
            <span className="text-sm text-slate-500">共 {chartData.length} 次复盘</span>
          </div>
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <span className="font-semibold text-slate-800">历史复盘记录</span>
        </div>
        ...
        {loadingInterview && (
          <div className="fixed inset-0 bg-black/20 flex items-center justify-center z-50">
            <div className="bg-white rounded-2xl p-6 flex items-center gap-4">
              <motion.div
                className="w-8 h-8 border-3 border-slate-200 border-t-amber-500 rounded-full"
                animate={{ rotate: 360 }}
                transition={{ duration: 1, repeat: Infinity, ease: "linear" }}
              />
              <span className="text-slate-600">加载复盘详情...</span>
            </div>
          </div>
        )}
      </motion.div>
  ...
  return (
    ...
    >
      ...
      <motion.button
        ...
        disabled={exporting}
          className="px-3 py-2 text-slate-400 hover:text-amber-500 hover:bg-white rounded-lg transition-all"
