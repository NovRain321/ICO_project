...
export default function AnalysisPanel({
  ...
  const getCategoryColor = (category: string) => {
    const colors: Record<string, string> = {
      '证据补充': 'bg-purple-100 text-purple-700',
      '法规引用': 'bg-indigo-100 text-indigo-700',
      '责任分析': 'bg-emerald-100 text-emerald-700',
      '处置建议': 'bg-pink-100 text-pink-700',
      '案情分析': 'bg-cyan-100 text-cyan-700',
      '程序规范': 'bg-orange-100 text-orange-700'
    };
    ...
    return (
      <div className="bg-white rounded-2xl p-12 text-center">
        ...
        <h3 className="text-xl font-semibold text-slate-700 mb-2">
          {isExplicitProcessing ? 'AI 正在研判中...' : '等待研判'}
        </h3>
        <p className="text-slate-500 mb-4">
          {isExplicitProcessing
            ? '请稍候，AI 正在对案件材料进行深度研判分析'
            : '案件材料已上传成功，即将开始 AI 研判'}
        </p>
        <p className="text-sm text-slate-400">页面将自动刷新显示研判结果</p>
      </div>
    );
  }
  ...
  if (analyzeStatus === 'FAILED' || !isAnalysisValid) {
    return (
      <div className="bg-white rounded-2xl p-12 text-center">
        ...
        <h3 className="text-xl font-semibold text-slate-700 mb-2">研判失败</h3>
        ...
        {onReanalyze && (
          <motion.button
            ...
            className="px-6 py-2.5 bg-amber-500 text-white rounded-xl font-medium hover:bg-amber-600 transition-colors disabled:opacity-50 flex items-center gap-2 mx-auto"
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
          >
            <RefreshCw className={`w-4 h-4 ${reanalyzing ? 'animate-spin' : ''}`} />
            {reanalyzing ? '重新研判中...' : '重新研判'}
          </motion.button>
        )}
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-2 text-slate-500">
              ...
              <span className="font-semibold">研判摘要</span>
            </div>
            <motion.button
              onClick={onExport}
              disabled={exporting}
            ...
            >
              ...
              {exporting ? '导出中...' : '导出研判报告'}
            </motion.button>
          </div>

          <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-xl p-6">
            <p className="text-lg text-slate-800 leading-relaxed mb-6">
              {analysis.summary || '案件材料已上传，等待 AI 研判分析。'}
            </p>

            <div className="grid grid-cols-2 gap-4 mb-4">
              <div className="bg-white rounded-xl p-5">
                <span className="text-sm font-semibold text-amber-600 block mb-2">总分</span>
                <span className="text-4xl font-bold text-slate-900">{analysis.overallScore || 0}</span>
                <span className="text-sm text-slate-500">/ 100</span>
              </div>
              <div className="bg-white rounded-xl p-5">
                <span className="text-sm font-semibold text-amber-600 block mb-2">研判时间</span>
            ...
            {analysis.strengths && analysis.strengths.length > 0 && (
              <div className="bg-white rounded-xl p-4">
                <span className="text-sm font-semibold text-amber-600 block mb-3">材料优势</span>
                <div className="flex flex-wrap gap-2">
                  {analysis.strengths.map((s: string, i: number) => (
                    <span key={i} className="px-3 py-1.5 bg-amber-100 text-amber-700 border border-amber-200 rounded-lg text-sm font-medium">
                      {s}
                    </span>
                  ))}
                </div>
        ...
        >
          <div className="flex items-center gap-2 text-slate-500 mb-6">
            ...
            <span className="font-semibold">研判维度评分</span>
          </div>
      ...
      >
        <div className="flex items-center gap-2 text-slate-500 mb-6">
          ...
          <span className="font-semibold">处置建议</span>
