...
export default function InterviewHistoryPage({ onBack: _onBack, onViewInterview }: InterviewHistoryPageProps) {
  ...
  const handleExport = async (sessionId: string, e: React.MouseEvent) => {
    ...
    try {
      ...
      a.href = url;
      a.download = `案件复盘报告_${sessionId.slice(-8)}.pdf`;
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-start mb-8 flex-wrap gap-6">
        ...
        <motion.div
          className="flex items-center gap-3 bg-white border border-slate-200 rounded-xl px-4 py-2.5 min-w-[280px] focus-within:border-amber-500 focus-within:ring-2 focus-within:ring-amber-100 transition-all"
          initial={{ opacity: 0, x: 20 }}
          animate={{ opacity: 1, x: 0 }}
        >
      ...
      {!loading && filteredInterviews.length === 0 && (
        ...
        >
          ...
          <p className="text-slate-500">开始一次案件复盘后，记录将显示在这里</p>
        </motion.div>
      )}
        ...
        >
          <table className="w-full">
            <thead className="bg-slate-50 border-b border-slate-100">
              <tr>
                <th className="text-left px-6 py-4 text-sm font-medium text-slate-600">关联案件</th>
                <th className="text-left px-6 py-4 text-sm font-medium text-slate-600">核查项数</th>
                      ...
                      <div className="flex items-center justify-end gap-1">
                        ...
                        {isEvaluateCompleted(interview) && (
                          <button
                            ...
                            className="p-2 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-lg transition-colors disabled:opacity-50"
                            title="导出PDF"
                          >
