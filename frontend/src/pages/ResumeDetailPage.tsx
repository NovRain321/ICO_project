...
export default function ResumeDetailPage({ resumeId, onBack, onStartInterview }: ResumeDetailPageProps) {
  ...
  const handleExportAnalysisPdf = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `警情研判报告_${resume?.filename || resumeId}.pdf`;
  ...
  const handleExportInterviewPdf = async (sessionId: string) => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `案件复盘报告_${sessionId}.pdf`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);
  ...
  const tabs = [
    { id: 'analysis' as const, label: '案情研判', icon: CheckSquare },
    { id: 'interview' as const, label: '复盘记录', icon: MessageSquare, count: resume.interviews?.length || 0 },
  ];
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-900">
              {detailView === 'interviewDetail' ? `复盘详情 #${selectedInterview?.sessionId?.slice(-6) || ''}` : resume.filename}
            </h2>
        ...
        <div className="flex gap-3">
          {detailView === 'interviewDetail' && selectedInterview && (
            ...
            >
              ...
              {exporting === selectedInterview.sessionId ? '导出中...' : '导出复盘报告'}
            </motion.button>
          )}
          {detailView !== 'interviewDetail' && (
            <motion.button
              onClick={() => onStartInterview(resume.resumeText, resumeId)}
              className="px-5 py-2.5 bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-xl font-medium shadow-lg shadow-amber-500/30 hover:shadow-xl transition-all flex items-center gap-2"
            ...
            >
              ...
              <span className="relative z-10 flex items-center gap-2">
                ...
                {tab.count !== undefined && tab.count > 0 && (
                  <span className="px-2 py-0.5 bg-amber-100 text-amber-600 text-xs rounded-full">{tab.count}</span>
                )}
              </span>
            </motion.button>
          ))}
