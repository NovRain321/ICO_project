  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-start mb-10 flex-wrap gap-6">
        ...
        <motion.div 
          className="flex items-center gap-3 bg-white border border-slate-200 rounded-xl px-4 py-3 min-w-[280px] focus-within:border-amber-500 focus-within:ring-2 focus-within:ring-amber-100 transition-all"
          initial={{ opacity: 0, x: 20 }}
          animate={{ opacity: 1, x: 0 }}
        >
      ...
      {loading && (
        <div className="text-center py-20">
          <motion.div 
            className="w-10 h-10 border-3 border-slate-200 border-t-amber-500 rounded-full mx-auto mb-4"
            animate={{ rotate: 360 }}
      ...
      {!loading && filteredResumes.length === 0 && (
        ...
        >
          ...
          <h3 className="text-xl font-semibold text-slate-700 mb-2">暂无案件记录</h3>
          <p className="text-slate-500">上传案件材料开始 AI 案情研判</p>
        </motion.div>
      )}
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                <th className="text-left px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wide">案件名称</th>
                <th className="text-left px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wide">上传日期</th>
                <th className="text-left px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wide">研判评分</th>
                <th className="text-left px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wide">研判状态</th>
                  ...
                  >
                    ...
                    <td className="px-6 py-5">
                      {resume.interviewCount > 0 ? (
                        ...
                        <span className="inline-flex px-3 py-1 bg-slate-100 text-slate-500 rounded-full text-sm">待研判</span>
                      )}
                    </td>
                    <td className="px-4">
                      <div className="flex items-center gap-2">
                        ...
                        <svg className="w-5 h-5 text-slate-300 group-hover:text-amber-500 group-hover:translate-x-1 transition-all" viewBox="0 0 24 24" fill="none">
      ...
      <DeleteConfirmDialog
        ...
        customMessage={
          deleteConfirm ? (
            <>
              <p className="mb-2">确定要删除案件 <strong>"{deleteConfirm.filename}"</strong> 吗？</p>
