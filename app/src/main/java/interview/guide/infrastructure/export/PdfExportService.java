...
/**
 * PDF导出服务
 * PDF Export Service for case analysis and review reports
 */
@Slf4j
@Service
...
public class PdfExportService {
    ...
    public byte[] exportResumeAnalysis(ResumeEntity resume, ResumeAnalysisResponse analysis) {
        ...
        // 标题
        Paragraph title = new Paragraph("警情研判报告")
            .setFontSize(24)
            .setBold()
        ...
        document.add(createSectionTitle("案件信息"));
        document.add(new Paragraph("案件名称: " + resume.getOriginalFilename()));
        ...
        if (analysis.scoreDetail() != null) {
            document.add(new Paragraph("\n"));
            document.add(createSectionTitle("研判维度评分"));

            Table scoreTable = new Table(UnitValue.createPercentArray(new float[]{2, 1}))
                .useAllAvailableWidth();
            addScoreRow(scoreTable, "案情复杂度", analysis.scoreDetail().projectScore(), 40);
            addScoreRow(scoreTable, "法规适配度", analysis.scoreDetail().skillMatchScore(), 20);
            addScoreRow(scoreTable, "材料完整性", analysis.scoreDetail().contentScore(), 15);
            addScoreRow(scoreTable, "描述清晰度", analysis.scoreDetail().structureScore(), 15);
            addScoreRow(scoreTable, "研判规范性", analysis.scoreDetail().expressionScore(), 10);
        ...
        if (analysis.summary() != null) {
            document.add(new Paragraph("\n"));
            document.add(createSectionTitle("案情摘要"));
        ...
        if (analysis.strengths() != null && !analysis.strengths().isEmpty()) {
            document.add(new Paragraph("\n"));
            document.add(createSectionTitle("材料优势"));
        ...
        if (analysis.suggestions() != null && !analysis.suggestions().isEmpty()) {
            document.add(new Paragraph("\n"));
            document.add(createSectionTitle("处置建议"));
    ...
    public byte[] exportInterviewReport(InterviewSessionEntity session) {
        ...
        // 标题
        Paragraph title = new Paragraph("案件复盘报告")
            .setFontSize(24)
            .setBold()
            .setTextAlignment(TextAlignment.CENTER)
            .setFontColor(HEADER_COLOR);
        ...
        // 基本信息
        document.add(new Paragraph("\n"));
        document.add(createSectionTitle("复盘信息"));
        document.add(new Paragraph("复盘ID: " + session.getSessionId()));
        document.add(new Paragraph("核查问题数: " + session.getTotalQuestions()));
        document.add(new Paragraph("复盘状态: " + getStatusText(session.getStatus())));
        ...
        if (session.getOverallFeedback() != null) {
            document.add(new Paragraph("\n"));
            document.add(createSectionTitle("综合评价"));
        ...
        if (session.getStrengthsJson() != null) {
            try {
                ...
                if (!strengths.isEmpty()) {
                    document.add(new Paragraph("\n"));
                    document.add(createSectionTitle("研判优势"));
        ...
        if (answers != null && !answers.isEmpty()) {
            document.add(new Paragraph("\n"));
            document.add(createSectionTitle("核查详情"));

            for (InterviewAnswerEntity answer : answers) {
                document.add(new Paragraph("\n"));
                document.add(new Paragraph("核查项 " + (answer.getQuestionIndex() + 1) +
                    " [" + (answer.getCategory() != null ? answer.getCategory() : "综合") + "]")
                    .setBold()
                    .setFontSize(12));
                document.add(new Paragraph("问题: " + sanitizeText(answer.getQuestion())));
                document.add(new Paragraph("回答: " + sanitizeText(answer.getUserAnswer() != null ? answer.getUserAnswer() : "未回答")));
